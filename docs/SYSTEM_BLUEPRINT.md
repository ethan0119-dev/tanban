...
| 商户老板 | 当前租户唯一店铺的全部权限；全局老板账号可关联多个独立租户并切换 |
...
当前落地版本采用“一店一租户”：店铺是独立数据与计费单元，不在一个租户内共享商品、顾客、会员或员工。`account` 是全局登录身份，`tenant_membership` 决定其在某个店铺租户中的角色；只有老板允许拥有多个 OWNER 成员关系，员工只属于一个租户。
...
- 每个商品支持 1 张主图和最多 3 张辅图；图片来自门店图片库，公开商品目录同步返回有序图库。
- 图片库按门店隔离，支持分组、搜索、上传、改名、移动和引用保护；商品、装修、弹窗广告及门店 Logo 复用同一份素材。首版每店最多 1000 张、合计 1 GiB，删除未引用的本地素材时同步回收文件。
- 保存图片引用与删除图片使用统一的门店行锁和 `ACTIVE` 状态复验，避免并发请求产生指向已删除文件的商品或装修配置。
- 温度、甜度等规格模板。
- 渠道价格、会员价、上下架、推荐、售罄、置满、复制和批量操作。
- 商品统计使用已支付订单口径，分别展示支付订单数、销售数量与退款前毛销售额，不能与退款后净收入混用。
- 库存总量、出摊库存、扣减和盘点流水。
- 标签打印模板和商品打印路由。
...
- 普通图片、启动页、图片热区和营销弹窗都可直接从门店图片库选择；被当前或历史配置引用的素材禁止删除。
- 导航、启动页、客服二维码和门店公告。
- 草稿、预览、发布、历史版本和回滚。

...
完整观摩清单已独立沉淀为 [《爨火 / FirePOS 商户运营后台菜单清单》](./REFERENCE_FIREPOS_MERCHANT_ADMIN_MENU.md)。该清单按成功登录后的真实 DOM 菜单树记录了 **10 个一级模块、88 个侧栏目标页面菜单**，并单独记录“应用列表”中的 **29 个功能卡片**及已经确认的页面页签和关键操作。

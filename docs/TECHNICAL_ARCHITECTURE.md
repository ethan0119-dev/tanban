...
| `background runner` | 当前在 API 进程内处理打印重试、支付查单和退款查单 | 不参与 HTTP 鉴权和页面响应；通知、日终对账和超时关单调度仍待补齐 |
...
3. 当前已有订单号、建单幂等键、Provider 交易号和单订单支付唯一约束；真实回调事件 ID 和更细的打印业务幂等键仍需在对应 Provider 落地时补齐。
...
| 定时任务 | 当前 API 内打印、支付与退款 Worker | 超时关单、通知、日报和多实例租约任务后续实现 |
...
- 所有时间以 UTC 保存，展示时转换到门店时区；保留支付渠道原始时间。

...
func (s *Server) platformRoutes(r chi.Router) {
	...
	r.Get("/settings/website", s.getPlatformWebsiteSettings)
	r.With(requireRoles(RolePlatformAdmin)).Put("/settings/website", s.updatePlatformWebsiteSettings)
	r.Get("/leads", s.listCustomerLeads)
	r.With(requireRoles(RolePlatformAdmin)).Put("/leads/{leadID}", s.updateCustomerLead)
	...
	r.Get("/wechat-onboarding/pending", s.listPendingWechatOnboarding)

...
func copyProduct(ctx context.Context, tx *sql.Tx, tenantID, storeID, productID int64, name string) (int64, error) {
	...
	result, err := tx.ExecContext(ctx, `INSERT INTO products(tenant_id,store_id,category_id,product_type,unit_resource_id,name,description,image_url,sort_order,recommended,member_discount_enabled,in_store_enabled,delivery_enabled,max_per_order,cashier_only,channels_json,sale_periods_json,print_label_resource_id,status)
		SELECT tenant_id,store_id,category_id,product_type,unit_resource_id,?,description,image_url,sort_order+1,0,member_discount_enabled,in_store_enabled,0,max_per_order,cashier_only,channels_json,sale_periods_json,print_label_resource_id,'DISABLED'

...
func TestPrintTemplateCopiesMigrationPreservesLegacyTemplatesAndAddsStructuredRoles(t *testing.T) {
	...
	for _, expected := range []string{
		"ADD COLUMN copy_role VARCHAR(24) NULL DEFAULT NULL",
		...
		"UNIQUE KEY uk_print_templates_role_v2 (tenant_id, store_id, business_type, template_type, copy_role_key)",
	...
	for _, expected := range []string{
		"DROP KEY uk_print_templates_role_v2",

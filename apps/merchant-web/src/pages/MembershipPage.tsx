        ...
        <Tabs activeKey={activeTab} onChange={setActiveTab} items={[
          ...
          },
          {
            key: 'settings', label: '会员卡设置', children: <Row gutter={24}>
              <Col xs={24} lg={9}><div className="member-card-preview" style={{ background: settings?.card_color || '#8b5635' }}><small>码农咖啡</small><strong>{Form.useWatch('card_name', settingsForm) || '会员卡'}</strong><span>NO. 00000001</span></div></Col>
              <Col xs={24} lg={15}><Alert type="info" showIcon message="自动开卡与消费成长值为规则预留" description="V1 支持保存会员卡样式、等级和人工开卡；自动规则要等可信支付回调和会员身份闭环接入后才会执行。" style={{ marginBottom: 16 }} />{settingsLoadError ? <Alert type="error" showIcon message="会员卡设置加载失败，已禁止保存" description={settingsLoadError} style={{ marginBottom: 16 }} /> : null}<Form form={settingsForm} layout="vertical"><Row gutter={12}><Col span={12}><Form.Item label="会员功能" name="enabled" valuePropName="checked"><Switch /></Form.Item></Col><Col span={12}><Form.Item label="首单自动开卡（预留）" name="auto_enroll" valuePropName="checked"><Switch disabled /></Form.Item></Col></Row><Row gutter={12}><Col span={12}><Form.Item label="卡名称" name="card_name" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={12}><Form.Item label="主题色" name="card_color"><Input type="color" /></Form.Item></Col></Row><Form.Item label="默认等级" name="default_level_id"><Select allowClear options={levels.filter((item) => item.status === 'ACTIVE').map((item) => ({ value: item.id, label: item.name }))} /></Form.Item><Form.Item label="每消费 1 元获得成长值（预留）" name="growth_per_yuan"><InputNumber disabled min={0} precision={0} style={{ width: '100%' }} /></Form.Item><Form.Item label="卡面图片 URL" name="card_image_url"><Input /></Form.Item><Form.Item label="会员协议 URL" name="agreement_url"><Input /></Form.Item><Form.Item label="小程序展示余额" name="show_balance" valuePropName="checked"><Switch /></Form.Item><Button type="primary" icon={<SettingOutlined />} loading={saving} disabled={!settingsReady} onClick={() => void saveSettings()}>保存设置</Button></Form></Col>
            </Row>,
          },
          {
            key: 'orders', label: '等级订单', children: <>
              <Alert type="info" showIcon message="V1 暂未接入小程序付费开卡" description="此处创建的是商户侧等级订单记录，不会发起会生活支付，也不会自动改变会员等级。" style={{ marginBottom: 16 }} />

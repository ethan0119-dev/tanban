...
- 商户端：经营总览、订单履约、商品/SKU/库存、分类/套餐/属性/加料/标签等配置库、顾客标签、会员等级与开卡、不可变余额流水、储值规则与记录、可发布/回滚的小程序装修、部分退款、打印机/打印任务、员工角色和门店设置。
- 顾客端：门店码识别、已发布店铺装修、商品目录、规格/属性/加料选择、服务端权威计价购物车、幂等下单、Mock 支付和订单状态。

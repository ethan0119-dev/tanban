...
> 当前交付状态（2026-07-20）：Mock 支付、关单、部分退款、幂等和主动补偿链路可运行；真实会生活/天阙适配器尚未获得合作方凭证和最终接口协议，仍会显式返回 `ErrNotConfigured`。本环境只能用于联调，不能用于真实营业收款。
...
同一订单的发起支付、关单和成功回调共用 MySQL 连接级命名锁，避免双击、重试和回调并行穿透。支付记录采用追加式尝试模型：首次拉起先写 `CREATING`；结果不确定时只以同一个 `provider_request_no` 重试/查单；明确 `CLOSED/FAILED` 后再次支付才新增一条尝试，并生成新的通道幂等号。旧尝试不会被覆盖，数据库以 `unique(provider, provider_request_no)` 和 `unique(provider, provider_order_no)` 防止通道单号复用。每次尝试都快照当时的 `mno/subAppid/OpenID`，不能在商户绑定变化后误投到新账户。若极端情况下不同尝试先后成功，订单进入 `PAYMENT_EXCEPTION` 并报警，必须人工核查和退款，不能静默当作重复回调。

支付机构侧必须以支付尝试号、退款号分别实现创建支付和退款幂等，否则无法覆盖“渠道成功、我方写库前进程退出”的极端窗口。接真实通道时，`CLOSED` 必须来自通道关单/查单的权威终态；验签回调落库前还要核对商户号、支付尝试号和金额。

...
export function DashboardPage() {
  ...
  const trendData = (data?.revenueTrend ?? []).map((item) => ({ name: item.label, 营业额: item.value }));
  const monthlyData = (data?.monthlyTrend ?? []).map((item) => ({ name: item.label, 营业额: item.value }));
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    ...
                    <Tooltip
                      ...
                      formatter={(value: any, name: any) => [`${value} 单`, name]}
                    />
                    <Legend
                      verticalAlign="bottom"
                      iconType="circle"
                      iconSize={8}
                      formatter={(value: any) => <span style={{ color: '#66584f', fontSize: 13 }}>{value}</span>}
                    />
                  </PieChart>
                </ResponsiveContainer>
              </div>
